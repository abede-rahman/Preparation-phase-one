...
function palindromeRecursive(sentence) {
    ...
    function innerRecursive(newSentence){
        if(newSentence.length === 1){
            return newSentence[0];
        }

        if(newSentence.length > 1){
            return newSentence[newSentence.length - 1] + innerRecursive(newSentence.substring(0, newSentence.length -1));
        }
    }

    return sentence === innerRecursive(sentence);
}
